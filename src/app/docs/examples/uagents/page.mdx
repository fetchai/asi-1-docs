...
[ASI-1](https://asi1.ai/) is an LLM created by Fetch.ai, unlike other LLMs ASI-1 connects to [Agents](https://fetch.ai/docs/uagents/docs) which act as domain experts allowing ASI-1 to answer specialist questions, make reservations and become an access point to an _organic_ multi-Agent ecosystem.
...
## Getting started
    ...
    - make sure you have [uAgents package installed](https://fetch.ai/docs/uagents/docs/getting-started/install).
    - Sign up to [Agentverse](https://www.agentverse.ai) so that you can create a [mailbox](https://fetch.ai/docs/uagents/docs/agentverse/mailbox) for your Agent.
...
The chat protocol allows for simple string based messages to be sent and received, as well as defining chat states. It's the expected communication format for ASI-1. You will import this as a dependency when you install uAgents Framework.
...
The most important thing to note about the chat protocol, is `ChatMessage(Model)`; this is the wrapper for each message we send, inside this, there is a list of `AgentContent` which can be a number of models, most often you'll probably be using `TextContent`. An example of this is a littler further down the page.
...
This is a pretty simple function to make a call to `asi1-mini`, to get a response to a prompt that we send. The function takes two args, `context` and `prompt`. Let's build the Agent, and see how we call `get_completion`.
...
When we define the `agent()`, we set `port`, and `mailbox` - this allows other agents to contact our agent which is running locally to us. We add `publish_agent_details` to tell agentverse about us. For more information on mailbox, check out this resource [here](https://fetch.ai/docs/uagents/docs/agentverse/mailbox).

We add an `on_event` handler, to output to log the agent's address. Check out these resources for [Agent Handlers](https://fetch.ai/docs/uagents/docs/guides/handlers) and [Addresses](https://fetch.ai/docs/uagents/docs/getting-started/address).
...
Let's click the link for the [Agent inspector](https://fetch.ai/docs/uagents/docs/agentverse/inspector), you should see a window like the below:

![](../../../../content/images/quickstart/inspector_quickstart.png)

click **Connect** and in the modal click **Mailbox**-

You'll then see instructions to update your Agent to use the mailbox:

![](../../../../content/images/quickstart/inspector_select.png)

Now our Agent can receive messages from any other Agent! You can read more about the mailbox [here](https://fetch.ai/docs/uagents/docs/agentverse/mailbox) and [here](https://agentverse.ai/docs/uagents/mailbox).
...
![](../../../../content/images/quickstart/landing.png)

Using the address of your Agent, search for your Agent.

![](../../../../content/images/quickstart/search.png)

Select your Agent, and then, ask your Agent a question 👍.

![](../../../../content/images/quickstart/start_chat.png)

Then you should see your Agents response in the chat window:

![](../../../../content/images/quickstart/response.png)

Your Agent should have an output log similar to the one shown below:
...
Of course, you could also create a client to your Agent and skip [chat.agentverse.ai](https://chat.agentverse.ai); here's an example of that too:
...
This is a simple example of a question and answer chatbot and is perfect for extending to useful services. [chat.agentverse.ai](https://chat.agentverse.ai) is the first step in getting your Agents onto ASI1, keep an eye on our [blog](https://fetch.ai/blog) for the future release date.
