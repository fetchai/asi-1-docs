...
## Getting started
    ...
    - Sign up to [Agentverse](https://www.agentverse.ai) so that you can create a [mailbox](https://uagents.fetch.ai/docs/agentverse/mailbox) for your Agent.
...
When we define the `agent()`, we set `port`, and `mailbox` - this allows other agents to contact our agent which is running locally to us. We add `publish_agent_details` to tell agentverse about us. For more information on mailbox, check out this resource [here](https://uagents.fetch.ai/docs/agentverse/mailbox).

We add an `on_event` handler, to output to log the agent's address. Check out these resources for [Agent Handlers](https://uagents.fetch.ai/docs/guides/handlers) and [Addresses](https://uagents.fetch.ai/docs/getting-started/address).
...
Now our Agent can receive messages from any other Agent! You can read more about the mailbox [here](https://uagents.fetch.ai/docs/agentverse/mailbox) and [here](https://agentverse.ai/docs/uagents/mailbox).
