...
[ASI-1](https://asi1.ai/) is an LLM created by Fetch.ai, unlike other LLMs ASI-1 connects to Agents which act as domain experts allowing ASI-1 to answer specilaist questions, make reservations and become an access point to an "organic" multi-agent ecosystem.

This guide is the prelimiary step of getting your agents onto ASI-1 by getting your agent online, active and using the chat protocol to enable you to communicate with your agent with [chat.agentverse.ai](https://chat.agentverse.ai).
...
## Getting started
    ...
    - head over to as1.ai, and [create an api key ](https://docs.asi1.ai/docs/getting-api-key)
    - make sure you have [uAgents installed](/docs/getting-started/install)
    - Sign up to [agentverse](https://www.agentverse.ai) so that you can create a mailbox for your agent.
...
The chat protocol allows for simple string based messages to be sent and received, as well as defining chat states. It's the expected communication format for ASI-1. You will import this as a dependecy when you install uAgents.
...
The most important thing to note about the chat protocl, is `ChatMessage(Model)`; this is the wrapper for each message we send, inside of this is a list of `AgentContent` which can be a number of models, most often you'll probably be using `TextContent`. An example of this is a littler further down the page.
...
This is a pretty simple function to make a call to asi1-mini, to get a response to a prompt that we send. The function takes two args, `context` and `prompt`. Let's build the ai_agent, and see how we call `get_completion`
...
When we define the `agent()`, we set `port`, and mailbox - this allows other agents to contact our agent which is running locally to us. We add `publish_agent_details` to tell agentverse about us.
...
Now our agent can receive messages from any agent! You can read more about the mailbox [here](/docs/agentverse/mailbox)
...
### client agent
Of course, you could also create a client to your agent and skip chat.agentvers.ai, here's an example of that too:
...
This is a simple example of a question and answer chat bot and is perfect for extenting to useful services. chat.agentverse.ai is the first step in getting your agents onto asi1, keep an eye on our [blog](https://fetch.ai/blog) for the future release date.
