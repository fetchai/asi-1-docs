...
The fast-paced startup environment and market makes it essential to correctly and efficiently validate a business idea quickly in order to attract users and investors. Fetch.ai [Agents](/docs) technology makes it possible to develop an application being able to evaluate startup ideas autonomously. By using **CrewAI** and **Agents** it is possible to design a **Startup Idea Analyzer** offering a structured way to assess the feasibility and validity of any business idea you may have. By leveraging agents, which are programmable micro-services designed to communicate with one another, this system can independently evaluate key aspects of your business including market demand, technological needs, and business strategies with the aim of providing a comprehensive evaluation of your startup idea.
...
- [Creating an agent](/docs/getting-started/agents)
- [Communicating with other agents](/docs/getting-started/communication)
- [Almanac Contract](/docs/almanac)
- [Register in Almanac](/docs/almanac/registration)
- [Utilising the Agentverse Mailroom service](https://fetch.ai/docs/guides/agentverse/utilising-the-mailbox)
- [Protocols](/docs/guides/protocols)
- [Agentverse Functions](/docs/agentverse/functions)
- [Register an Agent Function on the Agentverse](https://fetch.ai/docs/guides/agentverse/agentverse-functions/registering-agent-services)

## Pre-requisites

    - **Python**: Download and install from [Python official website](https://www.python.org/downloads/).
    - **Poetry**: Install by following the instructions on [Poetry's official website](https://python-poetry.org/docs/#installation).
    - **Gemini API key**: To get the Gemini API key, sign up at [Google AI Studio](https://aistudio.google.com/app/apikey) and generate the API key.
    - **Mailbox API key**: You can get the Mailbox API key by following the [Mailbox](/docs/agentverse/mailbox) guide.
...
<DocsCode local={true}>
    ...
    The `Agent` is defined using a seed phrase for wallet generation and a [Mailbox](/docs/agentverse/mailbox) for message exchanges. Then, a protocol named `Startup idea Analyzer version` (`version 0.1.2`) defines how the agent processes incoming messages. The agent uses this protocol to respond to requests containing startup idea descriptions. The agent relies on a component called `MarketResearchProcess` from an external module `crew_ai` to conduct market research on the provided startup idea. This research process is triggered upon receiving a startup idea description. The `StartUpIdeaAnalyzer` class defines a data model for startup idea descriptions; this model expects a field `description` that holds the string of text describing the startup idea provided by the user.
...
To run the example, make sure your scripts are running on your end and then head over to [DeltaV](https://fetch.ai/docs/concepts/ai-engine/deltav) and provide a description for your startup idea in the dedicated bar. Choose the [AI Engine personality](https://fetch.ai/docs/concepts/ai-engine/ai-engine-personalities) and click on **Start** button. You should get something similar to the following:

![](../../../../content/images/guides/uagents/crewai/crewAi-example-1.png)

![](../../../../content/images/guides/uagents/crewai/crewAi-example-2.png)
