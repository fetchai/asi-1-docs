...
Once you've [installed](/docs/getting-started/install) the uAgents Framework library, it's quite simple to get a minimal use case running.
...
2. We then need to import the `Agent` and `Context` classes from the `uagents` library, and then create an agent using the class `Agent`:
   ...
   It is optional but useful to include a `seed` parameter when creating an agent to set fixed [addresses](/docs/getting-started/address). Otherwise, random addresses will be generated every time you run the agent. Your address is kind of important, as this is how other agents will identify you.
