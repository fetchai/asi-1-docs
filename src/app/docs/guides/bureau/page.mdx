...
Next, define the behavior of each Agent, such as sending and receiving messages. This is done using decorators like `on_interval` to perform scheduled tasks and `on_message` to respond to incoming messages. Check out the [Agent Handlers](/docs/guides/handlers) for additional information on Agent Handlers and their usage.
