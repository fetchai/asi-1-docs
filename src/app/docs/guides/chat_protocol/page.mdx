...
This guide walks you through the implementation of agent-to-agent communication using protocols. It includes registering protocols in an [Almanac](/docs/almanac) contract and understanding their metadata, which is stored as manifests.

## What is Protocol?

A [protocol](/docs/guides/protocols) is a self-contained collection of message handlers. Its digest ensures that the protocol reliably supports the intended messages, serving as a unique identifier for the specific implementation.
...
[Handlers](/docs/guides/handlers) define the interaction logic:
