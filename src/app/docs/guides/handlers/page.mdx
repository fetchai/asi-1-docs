...
<DocsCode local={true}>
   ...
   It is optional but useful to include a `seed` parameter when creating an agent to set fixed [addresses](/docs/getting-started/address). Otherwise, random addresses will be generated every time you run the agent.
...
```
    ...
    <Callout type="info" emoji="ℹ️">
      After the first registration in the [Almanac](/docs/almanac) smart contract, it will take about 5 minutes before the agents can be found through the protocol.
    </Callout>
...
The `on_query()` handler is used to register a [Function](/docs/agentverse/functions) as a handler for incoming queries that match a specified `Model`. This decorator enables the agent to respond to queries in an event-driven manner.
